# v2.1.0 (2026-01-12)
 - Feature: A NewSplitLogger function was added that creates a logger
   that logs to two different loggers, depending on the log level.
   This type of logger is useful for separating logs into stdout and
   stderr when running as a service so that the service logs can be
   easily filtered.

# v2.0.1 (2025-11-15)
 - Feature: A new `GroubConfig.IgnoreSignalError` boolean was added.
   When set to `true`, the error generated by the signal handler will
   not be returned from the `group.Wait` call. This makes it possible
   to gracefully terminate an application using a signal and have that
   application exit without error.

 - Change: The dcontext.WithoutCancel was removed. It's no longer
   needed because context.WithoutCancel has been added to the runtime
   library.

 - Change: The package name was changed from github.com/datawire/dlib
   to github.com/telepresenceio/dlib/v2 and the project was forked
   into the telepresenceio organization. The reason being that nobody
   responded to issues and pull-requests at datawire.

 - Change: The OptimizedLogger interface was removed in favor of
   making the Logger interface optimized and adding a default
   implementation to make it easy to create a simple logger.
